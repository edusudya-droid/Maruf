...
async def on_channel_post(message: Message) -> None:
    logger.info(
        "channel_post keldi: chat_id=%s, expected=%s, message_id=%s",
        message.chat.id,
    ...
    if message.chat.id != app_settings.official_channel_id:
        logger.warning(
            "channel_id mos kelmadi: %s != %s — post o'tkazib yuborildi",
            message.chat.id,
            app_settings.official_channel_id,
        )
        return
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            logger.info("Post saqlandi: message_id=%s", message.message_id)
    except Exception as exc:
        logger.exception("Post saqlashda xato: %s", exc)
